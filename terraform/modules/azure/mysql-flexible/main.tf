resource "azurerm_subnet" "mysql-vnet-private-subnet" {
  ...
  name                 = var.mysql-properties.mysql-vnet-subnet-name
  virtual_network_name = var.vnet-name
  address_prefixes     = var.mysql-properties.mysql-vnet-subnet-address-prefixes
...
resource "azurerm_private_dns_zone" "mysql-private-dns-zone" {
  ...
  name = var.mysql-properties.mysql-private-dns-zone-name
}

resource "azurerm_private_dns_zone_virtual_network_link" "mysql-private-dns-zone-virtual-network-link" {
  resource_group_name = var.resource-group-properties.rg-name

  name                  = var.mysql-properties.mysql-private-dns-zone-virtual-network-link-name
...
resource "azurerm_mysql_flexible_server" "mysql-flexible-server" {
  ...
  name                   = var.mysql-properties.mysql-flexible-server-name
  administrator_login    = var.mysql-properties.mysql-administrator-login
  administrator_password = var.mysql-properties.mysql-administrator-password
  ...
  sku_name               = var.mysql-properties.mysql-sku-name
  version                = var.mysql-properties.mysql-version
...
resource "azurerm_mysql_flexible_database" "mysql-flexible-database" {
  ...
  name        = var.mysql-properties.mysql-flexible-database-name
